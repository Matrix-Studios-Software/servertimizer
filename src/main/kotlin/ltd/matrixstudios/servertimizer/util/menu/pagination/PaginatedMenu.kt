package ltd.matrixstudios.servertimizer.util.menu.pagination

import ltd.matrixstudios.servertimizer.util.menu.Button
import ltd.matrixstudios.servertimizer.util.menu.MenuController
import ltd.matrixstudios.servertimizer.util.menu.navigation.PageNavigationMenu
...
abstract class PaginatedMenu(
    ...
    open fun getExtraPageButtons() : MutableMap<Int, Button>
    {
        return mutableMapOf()
    }
    ...
    open fun updateMenu() {
        ...
        for (entry in getExtraPageButtons()) {
            inventory.setItem(entry.key, entry.value.constructItemStack(player))
        }
